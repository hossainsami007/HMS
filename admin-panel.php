...
if (isset($_POST['app-submit'])){
  ...
  $appdate = $_POST['appdate']; // YYYY-MM-DD
  $apptime = $_POST['apptime']; // HH:MM:SS

  //basic guard
  if (empty($appdate) || empty($apptime)) {
    echo "<script>alert('Please choose both date and time.');</script>";
  } else {
    // ✅ compare combined datetime vs now
    ...
    if ($selectedDateTime <= $now) {
        ...
        if (mysqli_num_rows($check_query) == 0) {
           ...
           ('$pid','$fname','$lname','$gender','$email','$contact','$doctor','$docFees','$appdate','$apptime','1','1')" 
            ...
            echo "<script>alert('The doctor is not available at that time/date. Please choose a different slot.');</script>";
    }
   }
  }
}
...
if (isset($_GET["generate_bill"])){
    ...
    if (!file_exists($tcpdfPath)) {
        echo "<script>alert('TCPDF is missing. Please place the TCPDF library in Hospital-ManagementPHP/TCPDF');</script>";
        exit;
    }
  ...
  $content = '<br/>';
  $content .= '<h2 align="center"> Hospital Management System</h2></br>';
  $content .= '<h3 align="center"> Bill</h3>';

  $content .= generate_bill();
  $obj_pdf->writeHTML($content);

    // ✅ avoid warnings if no buffer
